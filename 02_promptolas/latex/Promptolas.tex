...
\midrule
1.\ rész & RTFC módszertan (6 lépés + határolók) & $\sim$25 perc \\
2.\ rész & 6 haladó technika (Few-shot, CoT, Persona, Negatív, Reflexion, Meta prompt) & $\sim$30 perc \\
3.\ rész & Zárófeladat (dokumentum-feldolgozás) & $\sim$15 perc \\
\midrule
\multicolumn{2}{l}{\textbf{Teljes blokk}} & \textbf{$\sim$70 perc} \\
\bottomrule
\end{tabularx}
...
\noindent\textit{Cél: Az RTFC-n túl további négy technika megismerése, amelyek összetettebb feladatok megoldásához szükségesek.}
...
Egy ruházati webshop havi eladási adatai:
...
%--- Meta-prompting ---
\subsection{F) Meta-prompting -- írasd meg a promptot az AI-val}

Ha nem tudod pontosan, hogyan fogalmazd meg a promptot, kérd meg az AI-t,
hogy írja meg helyetted. Megadod a célt — az AI elkészíti az RTFC-alapú promptot,
amelyet aztán azonnal használhatsz.

\lbl{1. példa -- prompt generálás nulláról}

\begin{prompt}
Szeretnék SWOT-elemzést készíteni egy kávézó közösségi médiás jelenlétéről.

Írj nekem egy hatékony, RTFC-alapú promptot ehhez a feladathoz.
\end{prompt}

Futtasd a kapott promptot egy új beszélgetésben, és hasonlítsd össze azzal,
amit te magad írtál volna. Ez segít megérteni, hogyan fogalmaz egy AI
strukturált, RTFC-kompatibilis promptot.

\lbl{2. példa -- prompt fejlesztése RTFC-alapon}

\begin{prompt}
Ez a promptom:

"""
Elemzd a webshopom adatait.
"""

Hogyan javítanád RTFC-alapon?
Írd meg a javított, RTFC-kompatibilis verziót is.
\end{prompt}

A túl általános prompt nem ad jó eredményt: nem mondja meg a szerepet,
a feladatot, a formátumot és a kontextust. A gyakorlat célja, hogy az AI
segítsen RTFC-alapon pontosabb promptot készíteni.

\lbl{3. példa -- több lépéses feladat}

\begin{prompt}
Feladatom: egy egyetemi kurzushoz gyakorlati feladatot készítek,
amelyben a hallgatók webshop-adatokon (eladások, termékkategóriák)
elemzik a trendeket Excelben vagy Pythonban.

Írj RTFC-alapú promptot, amely:

- világosan megadja a szerepet (hallgató / elemző),
- a feladatot (trendek vizsgálata),
- a formátumot (rövid elemzés + táblázat vagy kód),
- a kontextust (webshop adatok).

A prompt legyen használható ChatGPT-vel vagy más LLM-mel.
\end{prompt}

\tip{Meta-prompting akkor hasznos, ha új területen dolgozol, vagy nem vagy biztos
abban, milyen szerepkört, formátumot adj meg. Az AI-t prompt-mérnökként is használhatod —
kérd meg, hogy ő fogalmazza meg a legjobb promptot.}
